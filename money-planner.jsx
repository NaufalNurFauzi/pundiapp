...
/* True "sisa saldo" — a running cash balance that rolls forward from month to
   ...
   all confirmed monthly incomes + all topups + all asset withdrawals (cash coming
   back in), minus all expenses (normal spending AND deposits into savings/assets,
   since those also leave your spendable cash). Category budgets still reset every
   month for planning purposes — this is only the overall cash total. */
function cumulativeBalanceUpTo(user, transactions, monthLimit) {
  ...
  const totalWithdraw = txUpTo.filter(isWithdrawTx).reduce((s, t) => s + Number(t.amount), 0);
  const totalExpense = txUpTo.filter(isExpenseTx).reduce((s, t) => s + Number(t.amount), 0);
  return totalIncome + totalTopup + totalWithdraw - totalExpense;
}
                  ...
                  <div className="flex flex-col gap-1.5 mt-3 pl-4" style={{ borderLeft: "2px solid var(--border)" }}>
                    {c.subs.map((s) => (
                      <div key={s.id} className="flex items-center justify-between text-sm muted">
                        <span>{s.name}</span>
                        <span className="tabular">{fmtPercent(s.percent)}% · {rupiah(amt * s.percent / 100)}</span>
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    <div className="mt-3 pl-4 flex flex-col gap-2 anim-fade-up" style={{ borderLeft: "2px solid var(--border)" }}>
                      ...
                      {c.subs.map((s) => (
                        <div key={s.id} className="flex items-center justify-between gap-2">
                          ...
                          <span className="tabular text-xs muted" style={{ minWidth: 90, textAlign: "right" }}>{rupiah(catAmount * s.percent / 100)}</span>
...
function TopBar({ name, onLogout, right }) {
  return (
    <div className="flex items-center justify-between p-4 md:p-6" style={{ borderBottom: "1px solid var(--border)" }}>
...
function Dashboard({ user, transactions, onOpenCategory, onLogout, onEditAlloc, onAddMoney, onViewHistory, onOpenSettings, onOpenGoals, onOpenCalculator, onConfirmMonthIncome }) {
  ...
  const effectiveIncome = income + totalTopup;
  const totalSpent = expenses.reduce((s, t) => s + Number(t.amount), 0);
  const thisMonthNet = effectiveIncome - totalSpent;
        ...
        <div className="card p-6">
          ...
          <div className="flex flex-wrap gap-8 mt-3">
            <div>
              ...
              <p className="display tabular" style={{ fontSize: 20, fontWeight: 700 }}>{rupiah(effectiveIncome)}</p>
        ...
        <div>
          ...
          <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))" }}>
            {user.categories.map((c, i) => {
              const allocated = effectiveIncome * c.percent / 100 + directTopupFor(c.id);
...
function CategoryDetail({ user, categoryId, transactions, onBack, onAddTx, onUpdateTx, onDeleteTx, onAddRecurring, onRemoveRecurring, onLogout }) {
  ...
  const allocated = baseAllocated + directTopup;
  const monthTx = allMonthTx.filter((t) => t.categoryId === cat.id);
  const spent = monthTx.filter(isExpenseTx).reduce((s, t) => s + Number(t.amount), 0); // "setor bulan ini" for asset categories
  ...
  const checkBalance = (amt, targetSubId) => {
    if (targetSubId) {
      ...
      if (sub) {
        ...
        const subRemaining = subAlloc - spentForSub(sub.id);
          ...
          {isAsset ? (
            <>
              <div className="flex flex-wrap gap-8">
                <div>
                  <p className="muted text-xs uppercase tracking-wide">Saldo Aset (total)</p>
                  <p className="tabular" style={{ fontWeight: 700, fontSize: 20, color: "var(--teal)" }}>{rupiah(catSaldo)}</p>
                </div>
                <div>
                  <p className="muted text-xs uppercase tracking-wide">Setor bulan ini</p>
                  <p className="tabular" style={{ fontWeight: 700 }}>{rupiah(spent)}</p>
                </div>
                <div>
              ...
              <p className="muted text-xs mt-3">
                Uang yang disetor ke sini bukan "hilang" seperti pengeluaran biasa — ini menambah saldo asetmu. Baru tercatat sebagai
                keluar (Tarik) saat kamu benar-benar mencairkan/memakainya.
              </p>
          ...
          <div className="card p-6">
            ...
            <div className="flex flex-col gap-3">
              {cat.subs.map((s, si) => {
                ...
                const subSpent = spentForSub(s.id);
                ...
                return (
                  <div key={s.id} className="card2 p-3 anim-fade-up" style={{ animationDelay: `${si * 60}ms` }}>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      {isAsset ? (
                        <span className="tabular" style={{ color: "var(--teal)" }}>Saldo {rupiah(subSaldo)}</span>
                      ) : (
                        <span className="tabular muted">{rupiah(subSpent)} / {rupiah(subAlloc)}</span>
                      )}
                    </div>
                    {isAsset && <p className="muted text-xs mb-1">Setor bulan ini: {rupiah(subSpent)} dari target {rupiah(subAlloc)}</p>}
            ...
            {filteredHistory.map((t, hi) => {
              ...
              return (
                <div key={t.id} className="card2 p-3 flex items-center justify-between anim-fade-up" style={{ animationDelay: `${Math.min(hi, 8) * 40}ms` }}>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="tabular" style={{ color: positive ? "var(--teal)" : "var(--rose)" }}>{positive ? "+" : "-"}{rupiah(t.amount)}</span>
...
function MonthlyHistory({ user, transactions, onBack, onUpdateTx, onDeleteTx }) {
  ...
  const effectiveIncome = selectedIncome + totalTopup;
  const totalSpent = expenses.reduce((s, t) => s + Number(t.amount), 0);
  const thisMonthNet = effectiveIncome - totalSpent;
        ...
        <div className="card p-6">
          <div className="flex flex-wrap gap-8">
            <div>
              ...
              <p className="tabular" style={{ fontWeight: 700 }}>{rupiah(effectiveIncome)}</p>
                  ...
                  {expanded && c.subs.length > 0 && (
                    <div className="mt-3 pl-4 flex flex-col gap-2" style={{ borderLeft: "2px solid var(--border)" }}>
                      {c.subs.map((s) => {
                        ...
                        const subSpent = spentForSub(c.id, s.id);
                        return (
                          <div key={s.id} className="text-sm">
                            <div className="flex justify-between muted mb-1">
                              <span>{s.name}</span>
                              <span className="tabular">{rupiah(subSpent)} / {rupiah(subAlloc)}</span>
                            </div>
            ...
            {filteredTx.map((t) => {
              ...
              return (
                <div key={t.id} className="card2 p-3 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="tabular" style={{ color: topup ? "var(--teal)" : "var(--rose)" }}>{topup ? "+" : "-"}{rupiah(t.amount)}</span>
                    <button className="icon-btn" onClick={() => setEditingTx(t)}><Pencil size={14} className="muted" /></button>
                    <button className="icon-btn" onClick={() => onDeleteTx(t.id)}><X size={15} className="muted" /></button>
                  </div>
                </div>
...
function GoalsPage({ user, onBack, onUpdateGoals, onLogout }) {
  ...
  const addGoal = () => {
    ...
    onUpdateGoals([...goals, { id: uid(), name: name.trim(), target: Number(target), saved: 0 }]);
    setName(""); setTarget("");
  };
  ...
  const contribute = (id, sign) => {
    ...
    if (!(amt > 0)) return;
    onUpdateGoals(goals.map((g) => g.id === id ? { ...g, saved: Math.max(0, g.saved + sign * amt) } : g));
    ...
    <div className="min-h-screen">
      ...
      <div className="p-4 md:p-8 max-w-2xl mx-auto flex flex-col gap-6 anim-fade-up">
        <div>
          ...
          <p className="muted text-sm mt-1">
            Bucket terpisah dari alokasi bulanan — cocok buat target jangka panjang seperti DP motor atau dana darurat.
            Nabung ke sini dicatat manual, tidak otomatis memotong alokasi kategori manapun.
          </p>
        </div>
        ...
        <div className="card p-6">
          ...
          <div className="flex gap-2 flex-wrap">
            <input style={{ flex: 1, minWidth: 160 }} placeholder="Nama tujuan (cth. DP Motor)" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={onEnter(addGoal)} />
            <input style={{ maxWidth: 160 }} type="number" placeholder="Target (Rp)" value={target} onChange={(e) => setTarget(e.target.value)} onKeyDown={onEnter(addGoal)} />
            <button className="btn-primary flex items-center gap-2" onClick={addGoal}><Plus size={16} /> Tambah</button>
          </div>
        </div>
      </div>
    </div>
